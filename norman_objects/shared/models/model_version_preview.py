    ...
    def validate_account_id(self):
        for asset in self.assets:
            if self.account_id != asset.account_id:
                raise ValueError("Model version preview account id does not match model asset account id")

    def validate_model_id(self):
        for asset in self.assets:
            if self.model_id != asset.model_id:
                raise ValueError("Model version preview model id does not match model asset model id")

    def validate_version_id(self):
        for asset in self.assets:
            if self.id != asset.version_id:
                raise ValueError("Model version preview id does not match model asset version id")
